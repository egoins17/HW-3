...
volBitcoin <- bitcoin %>% 
  ...
  mutate(avg_lag = lag(Avg)) %>%
  mutate(diff = Avg-avg_lag) %>%
  mutate(Volatility = diff/Avg) %>%
...
volGold <- gold %>% 
  ...
  mutate(diff = Value-value_lag) %>%
  mutate(Volatility = diff/Value) %>%
  mutate(Currency = "Gold") %>%
  select(Date, Volatility, Currency) %>%
  filter(!is.na(Volatility))

volatility <- rbind(volGold, volBitcoin) 
ggplot(data = volatility, aes(x = Date, y = Volatility, color = Currency)) + geom_line()   + ggtitle("Volatility of Gold and Bitcoin 2010-2016")
