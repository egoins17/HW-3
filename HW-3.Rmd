...
set.seed(76)
training <- profiles %>%
  sample_n(2997, replace = FALSE, weight = NULL, .env = parent.frame())
  
test <- profiles %>%
  sample_n(56946, replace = FALSE, weight = NULL, .env = parent.frame())
...
```{r, echo=TRUE, message=TRUE, warning=TRUE, fig.width=8, fig.height=4.5}
  predict_sex_model <- glm(is_female ~ 1, data = training, family = "binomial")
...
predict(predict_sex_model, newdata = test, type="response")
...
ggplot(data = gold, aes(x = Date, y = Value)) + geom_line()
